 ...
 * 
 * ServerToolKits.java at 2021-6-29 10:24:09, code by Jack Jiang.
 */
...
{
    /**
     * 设置MobileIMSDK核心框架的TCP协议心跳频率模式（默认为 {@link SenseModeTCP#MODE_15S} ）。
     * <p>
    ...
    /**
     * 设置MobileIMSDK核心框架的UDP协议心跳频率模式（默认为 {@link SenseModeUDP#MODE_3S} ）。
     * <p>
     * 请在 {@link net.x52im.mobileimsdk.server.ServerLauncher#startup()}方法被调用前被设置，否则将不起效.
     * <p>
     * <b>重要说明：</b><u>服务端本模式的设定必须要与客户端的模式设制保持一致</u>，否则
    ...
    /**
     * 设置MobileIMSDK核心框架的WebSocket协议心跳频率模式（默认为 {@link SenseModeWebsocket#MODE_15S} ）。
     * <p>
     * 请在 {@link net.x52im.mobileimsdk.server.ServerLauncher#startup()}方法被调用前被设置，否则将不起效.
     * <p>
     * <b>重要说明：</b><u>服务端本模式的设定必须要与客户端的模式设制保持一致</u>，否则
...
//		logger.debug("[IMCORE]>> 【收到数据长度】"+jsonStr.length()+", 内容："+jsonStr+".");
	 ...
	 * <font color="red">本方法将由MobileIMSDK框架内部算法按需调用，目前不建议也不需要开发者调用。</font>
	 * 
	 * @param protocalJSONStr Protocal对象的JSON文本形式
	 * @return 解析后的MobileIMSDK的完整协议内容（Protocal对象）
	 * @throws Exception 解析出错则抛出本异常
	 */
